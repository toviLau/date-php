 ...
 */
import getLunar from "../lunar/getLunar";
...
const getFestival = (dateObj: string, date: DateFunction): FestivalList => {
    const match = dateObj.match(/(\d{4})(\d{2})(\d{2})/);
    if (!match) return { cn: [], en: [] };
    const dateArr = match;
    const curDate = new Date(`${dateArr[1]}-${dateArr[2]}-${dateArr[3]}`);

    const getWeekInMonth = (): number => {
        const d = curDate.getDate();
        const w = curDate.getDay();
        return Math.ceil((d + 6 - w) / 7) - 1;
    };

    const lunarResult = getLunar.solar2lunar(Number(dateArr[1]), Number(dateArr[2]), Number(dateArr[3]));
    if (typeof lunarResult === "number") return { cn: [], en: [] };
    ...
    const getDate: string[] = [
        ...
        "@" +
            pad(
                Math.ceil(
                    (new Date(Number(dateArr[1]), Number(dateArr[2]) - 1, Number(dateArr[3])).getTime() -
    ...
    let holiday: Record<string, string[] | "" | undefined> = {
        "0101": ["元旦节", "New year"],
